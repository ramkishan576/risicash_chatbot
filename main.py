...
def fetch_travel_packages(query: str) -> Dict[str, Any]:
    ...
    """
    try:
        destinationName = re.search(r"destination:([\w\s]+)", query)
        categoryName = re.search(r"category:([\w\s]+)", query)
        destinationName = destinationName.group(1).strip() if destinationName else ""
        categoryName = categoryName.group(1).strip() if categoryName else ""
